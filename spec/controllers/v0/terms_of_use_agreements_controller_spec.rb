...
RSpec.describe V0::TermsOfUseAgreementsController, type: :controller do
  ...
  describe 'GET #latest' do
    let(:terms) { create(:terms_of_use_agreement, user_account:) }

    it 'returns the latest terms of use agreement for the user' do
      get :latest, params: { version: terms.agreement_version }
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)['terms_of_use_agreement']['id']).to eq(terms.id)
    end
  end
  ...
  describe 'POST #accept' do
    context 'when the agreement is accepted successfully' do
      ...
      it 'returns the accepted terms of use agreement' do
        ...
        expect(Rails.logger).to have_received(:info).with(
          ...
          hash_including(:terms_of_use_agreement_id, :user_account_id, :icn, :agreement_version, :response)
        )
      end
    end
    ...
    context 'when the agreement acceptance fails' do
      before do
        allow_any_instance_of(TermsOfUseAgreement).to receive(:accepted!).and_return(false)
      end
  ...
  describe 'POST #decline' do
    ...
    context 'when the agreement declination fails' do
      before do
        allow_any_instance_of(TermsOfUseAgreement).to receive(:declined!).and_return(false)
      end
