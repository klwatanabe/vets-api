...
module BGS
  class Children
    attr_reader :children, :step_children, :proc_id, :ssn, :icn, :common_name, :views, :dependents_application

    def initialize(proc_id:, payload:, icn:, common_name:, ssn:)
      @children = []
      @step_children = []
      @proc_id = proc_id
      @icn = icn
    ...
    def report_children
      dependents_application['children_to_add'].each do |child_info|
        ...
        bgs_service.create_person(person_params: person_params(child, participant, formatted_info))
    ...
    def report_stepchildren
      dependents_application['step_children'].each do |stepchild_info|
        ...
        participant = bgs_service.create_participant(proc_id:, ssn:)
        guardian_participant = bgs_service.create_participant(proc_id:, ssn:)

        step_child_guardian_person(guardian_participant, stepchild_info)
        bgs_service.create_person(person_params: person_params(step_child, participant, formatted_info))
    ...
    def report_child_event(event_type)
      ...
      bgs_service.create_person(person_params: person_params(child_event, participant, formatted_info))
    ...
    def step_child_parent(child_info)
      parent = bgs_service.create_participant(proc_id:, ssn:)
