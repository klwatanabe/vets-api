...
module VANotify
  class ConfirmationEmail < Common::RedisStore
    attribute :user_account_uuid_and_form_id
    ...
    redis_key :user_account_uuid_and_form_id

    def self.send(email_address:, template_id:, first_name:, user_account_uuid_and_form_id:)
      return if find(user_account_uuid_and_form_id)

      create(user_account_uuid_and_form_id:)
